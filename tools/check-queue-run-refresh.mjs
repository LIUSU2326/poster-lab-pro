...
for (const token of [
  "QUEUE RUN",
  "serviceFlow.queueRun",
  "serviceFlow.workspaceReload",
]) {
  if (!taskChrome.includes(token)) issues.push(`task-chrome.js: missing queue run status token ${token}`);
}
...
async function runRuntimeCheck() {
  ...
  if (calls.length !== 6) issues.push("HTTP queue refresh flow should make six route calls");
  const expectedMethods = ["POST", "POST", "POST", "POST", "POST", "GET"];
