...
if (!taskChrome.includes("submission.transport")) {
  issues.push("task-chrome.js: task drawer should expose submission transport");
}
...
async function runRuntimeCheck() {
  ...
  const expectedOrder = ["/snapshot", "/prompts", "/provider-requests", "/queue-plans", "/run", `/api/workspaces/${snapshot.metadata.workspaceId}`];
  ...
  if (calls.slice(0, 5).some((call) => call.method !== "POST")) issues.push("HTTP generation service should use POST before workspace reload");
  if (calls[5]?.method !== "GET") issues.push("HTTP generation service should use GET for final workspace reload");
  if (flow.queueRun?.data?.resultCount !== 4) issues.push("HTTP flow should expose queue run result count");
  if (!flow.workspaceReload?.ok) issues.push("HTTP flow should reload the workspace after queue run");
}
