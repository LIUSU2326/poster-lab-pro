...
function imagePrompt(request: ImageGenerationRequest): string {
  ...
  const posterInstruction = hasPosterMode
      ? [
        ...
        posterTextEconomyLock(),
        posterInWorldBrandTreatmentLock(),
        "The poster must feel designed as a game marketing key visual: strong depth or clean graphic clarity, deliberate light, expressive character acting, project-native objective/emotion/gameplay pressure, atmospheric or graphic polish, and a clear player promise.",
        "Breathtaking campaign brief: complex project-native storytelling, foreground elements framing the shot, dynamic character/core-mechanic/environment interaction, genre-appropriate atmosphere, VFX/particles when useful, project-specific details, rim-light pockets, and premium color grading.",
        "Campaign escalation must come from the set piece or format: asymmetrical low-angle, forced-perspective camera, intimate premium close-up, clean gameplay-proof surface, character lineup, one dominant action/comparison path, practical light source, rim/back light, volumetric beams, dust/smoke/particles when appropriate, or visible before-after transformation. Never solve quality by redesigning the uploaded characters into different people.",
        "Use a deliberate campaign composition architecture, not a default side-scrolling scene. Favor one of these KV structures when it fits the scheme: dynamic split-world contrast, portal/breach reveal, foreground prop divider, character lineup, gameplay-proof board/level surface, cozy room/shop/farm format, world/setting reveal, comic-panel mission montage, giant-scale project terrain, or reward/progression payoff.",
...
function briefPrompt(request: BriefGenerationRequest): string {
  ...
  return [
    ...
    JSON.stringify({
      ...
      schemeCount: request.schemeCount,
      randomizationSeed,
      adaptivePosterDiversitySlots: posterKvArchitectureBriefSlots(request.schemeCount, randomizationSeed),
      assets,
      rules: [
        "Use adaptivePosterDiversitySlots in order as anti-repetition planning scaffolds: scheme 1 considers slot 1, scheme 2 considers slot 2, and so on. These slots are not fixed scene templates and must never override the user's project description, focus guidance, uploaded assets, genre, tone, or core loop.",
        "For each scheme, first infer a project-native poster promise from projectName, gameDescription, focusGuidance, creativeDirection, and assets: e.g. cozy healing, character lineup, gameplay proof, puzzle aha moment, progression reward, collection showcase, world hook, event/social moment, management pressure, sports rivalry, horror suspense, romance/relationship beat, boss encounter, objective crisis, or brand-icon thumbnail. Then adapt the slot's composition scaffold only if it helps that promise.",
        ...
        "Treat focusGuidance as a soft creative emphasis, not a literal mandatory scene. It must never override the user's project brief, uploaded asset identity, story clarity, or KV quality.",
        "If focusGuidance mentions giant scale, micro perspective, cozy, character, gameplay, collection, romance, puzzle, or any other emphasis, translate it into a project-native poster promise and vary the scene architecture. Do not default every scheme to a flat side-view battlefield.",
        ...
        "Every scheme brief must include a concrete shot blueprint: foreground framing, uploaded hero performance when available, project-native focus, world/gameplay context, logo/copy safe area, and camera angle.",
        ...
        "At least one uploaded hero must physically interact with the core mechanic, objective, environment, prop, other character, or BOSS/threat when present: blocking, climbing, striking, sliding, casting, repairing, piloting, pulling, decorating, serving, solving, collecting, defending, or causing visible impact. Do not place heroes as symmetrical floating stickers around a central subject.",
        ...
        "Every scheme must use or adapt its assigned composition scaffold to prevent repetition, but the final concept must be chosen from the current project's genre, loop, tone, and user focus rather than from a fixed scenario list.",
        posterKvArchitectureDiversityRequirement(),
        "Use divergent project-native poster archetypes across the batch. Depending on the brief this may include character portrait/lineup, cozy healing mood, gameplay proof, board/level state, collection showcase, world/setting reveal, event/social moment, puzzle aha moment, progression reward, management pressure, sports rivalry, horror suspense, romance/relationship beat, boss encounter, objective crisis, or brand-icon thumbnail.",
        "Batch diversity hard lock: each scheme brief must explicitly name a different posterPromise, userClickReason, visual hook, setting/format, camera grammar, subject role, and emotional beat. No two schemes may share the same central set-piece or the same 'hero faces BOSS in a fiery tunnel/arena' template.",
        "At most one scheme in a batch may be a direct frontal hero-versus-BOSS showdown, and only if the project brief supports combat or BOSS pressure. Other schemes must visibly sell different player promises from the actual project brief.",
