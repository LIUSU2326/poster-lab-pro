...
export function localAssetUploadBaseDir(): string {
  const explicitDir = process.env.POSTER_LAB_UPLOAD_DIR?.trim();
...
export async function writeLocalAssetBinary(input: LocalAssetBinaryUploadInput): Promise<LocalAssetBinaryUploadResult> {
  ...
  const uploadRoot = path.join(localAssetUploadBaseDir(), "workspaces", workspaceSegment);
