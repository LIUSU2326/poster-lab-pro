...
export const promptGuardrailsByMode: Record<ProductionMode, PromptGuardrailRule[]> = {
  poster: [
    ...
    rule({
      ...
      rule: "The poster should read as a premium game campaign key visual: coherent trailer-moment story beat, strong protagonist/objective/BOSS focal hierarchy, cinematic lighting, depth, polished color grading, and campaign-ready composition where logo/copy supports the action.",
      negativeRule: "Avoid text-dominated layouts, stacked right-side copy walls, flat collage, sticker-like pasted assets, floating isolated elements, cheap clip-art, low-fidelity wallpaper composition, and tabletop food surface used as the entire scene.",
    }),
    rule({
      id: "poster-integrated-typography",
      mode: "poster",
      severity: "hard",
      rule: "When slogan mode is active, make the slogan a single visible campaign-copy treatment: exact custom game-campaign lettering integrated with the scene or logo style when possible; otherwise reserve a polished blank safe area for later copy.",
      negativeRule: "No silent omission of the copy area, duplicate slogan plaque, lower-right repeated subtitle, plain system-font text, PPT-like headline overlay, broken generated spelling, or text pasted on top without lighting/shadow integration.",
    }),
    rule({
